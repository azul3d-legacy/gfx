...
func (c *gl2Context) ConvertPrimitive(p gfx.Primitive) int {
	switch p {
	case gfx.Triangles:
		return gl.TRIANGLES
	case gfx.Points:
...
func (c *gl2Context) UnconvertFaceCull(fc int) gfx.FaceCullMode {
	switch fc {
	case gl.FRONT:
...
func (c *gl2Context) ConvertStencilOp(o gfx.StencilOp) int {
	switch o {
	case gfx.SKeep:
		return gl.KEEP
	case gfx.SZero:
...
func (c *gl2Context) UnconvertStencilOp(o int) gfx.StencilOp {
	switch o {
	case gl.KEEP:
		return gfx.SKeep
	case gl.ZERO:
...
func (c *gl2Context) ConvertCmp(cmp gfx.Cmp) int {
	switch cmp {
	case gfx.Always:
		return gl.ALWAYS
	case gfx.Never:
...
func (c *gl2Context) UnconvertCmp(cmp int) gfx.Cmp {
	switch cmp {
	case gl.ALWAYS:
		return gfx.Always
	case gl.NEVER:
...
func (c *gl2Context) ConvertBlendOp(o gfx.BlendOp) int {
	switch o {
	case gfx.BZero:
		return gl.ZERO
	case gfx.BOne:
...
func (c *gl2Context) UnconvertBlendOp(o int) gfx.BlendOp {
	switch o {
	case gl.ZERO:
		return gfx.BZero
	case gl.ONE:
...
func (c *gl2Context) ConvertBlendEq(eq gfx.BlendEq) int {
	switch eq {
	case gfx.BAdd:
		return gl.FUNC_ADD
	case gfx.BSub:
...
func (c *gl2Context) UnconvertBlendEq(eq int) gfx.BlendEq {
	switch eq {
	case gl.FUNC_ADD:
		return gfx.BAdd
